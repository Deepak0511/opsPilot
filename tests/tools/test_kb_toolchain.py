...
def test_count_knowledge_base_articles_tool(mocker):
    ...
    result = count_knowledge_base_articles_tool.invoke({"keyword": "Reset"})
    assert result == 2
    mock_count.assert_called_once_with(keyword="Reset", category=None, incident_id=None, tag=None)
...
def test_search_knowledge_base_tool_by_title(mocker, mock_kb):
    mock_search = mocker.patch("ops_pilot.tools.kb_toolchain.kb_repository.search_knowledge_base_by_keyword")
    mock_search.return_value = [mock_kb]
    
    results = search_knowledge_base_tool.invoke({"keyword": "Reset"})
...
def test_search_knowledge_base_tool_multiple_filters(mocker, mock_kb):
    mock_search_title = mocker.patch("ops_pilot.tools.kb_toolchain.kb_repository.search_knowledge_base_by_keyword")
    ...
    results = search_knowledge_base_tool.invoke({"keyword": "Reset", "category": "Auth"})
    assert len(results) == 1
    
def test_search_knowledge_base_tool_not_found(mocker):
    mocker.patch("ops_pilot.tools.kb_toolchain.kb_repository.search_knowledge_base_by_keyword", return_value=[])

    result = search_knowledge_base_tool.invoke({"keyword": "Unknown"})
    assert isinstance(result, str) and result.startswith("Validation Error")
